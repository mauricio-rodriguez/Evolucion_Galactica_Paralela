...
struct Force_dev{
	...
	__device__ Force_dev(){
		...
		jrk[0] = jrk[1] = jrk[2] = snp[0] = snp[1] = snp[2] =  0.f;
	}
};
	...
	for(int j=jstart; j<jend; j+=NTHREADS){
		...
		if(jend-j < NTHREADS){
			for(int jj=0; jj<jend-j; jj+=nskip){
				...
				if(jj+joff < jend-j) h6_kernel(ip, jp, fo, eps2);
			}
		}else{
#if 0
#pragma unroll
			...
			for(int jj=0; jj<NTHREADS; jj+=4*nskip){
				...
				h6_kernel(ip, jp0, fo, eps2);
				h6_kernel(ip, jp1, fo, eps2);
				h6_kernel(ip, jp2, fo, eps2);
				h6_kernel(ip, jp3, fo, eps2);
			}
#endif
		}
	}
