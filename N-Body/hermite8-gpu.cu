...
__device__ Force_dev(){
	...
	jrk[0] = jrk[1] = jrk[2] = snp[0] = snp[1] = snp[2] =  crk[0] = crk[1] = crk[2] = 0.f;
}
};
...
float gamma = (3.f*dvda + drdj) * rinv2 
			 + alpha * (3.f*beta - 4.f*(alpha*alpha));
// rinv1 *= jp.mass;
rinv1 *= jp.posH.w;
...
float sx = rinv3*dax + (-6.f*alpha)*jx + (-3.f*beta)*ax;
float sy = rinv3*day + (-6.f*alpha)*jy + (-3.f*beta)*ay;
float sz = rinv3*daz + (-6.f*alpha)*jz + (-3.f*beta)*az;
...
for(int j=jstart; j<jend; j+=NTHREADS){
	...
	if(jend-j < NTHREADS){
		for(int jj=0; jj<jend-j; jj+=nskip){
			...
			if(jj+joff < jend-j) h8_kernel(ip, jp, fo, eps2);
		}
	}else{
#if 0
#pragma unroll
		...
		for(int jj=0; jj<NTHREADS; jj+=4*nskip){
			...
			h8_kernel(ip, jp0, fo, eps2);
			h8_kernel(ip, jp1, fo, eps2);
			h8_kernel(ip, jp2, fo, eps2);
			h8_kernel(ip, jp3, fo, eps2);
		}
#endif
	}
}
...
cudaFuncSetCacheConfig(h8_gravity,    cudaFuncCachePreferShared);
